...
====================================================*/
	...
	SELECT COUNT(BillingState)   --NULL sayılması için COUNT(*)KULLAN.--
	FROM invoices
	
	SELECT COUNT(DISTINCT Composer) AS besteci_sayısı
	FROM tracks;
	
	SELECT COUNT(Composer) AS besteci_sayısı
	FROM tracks;
	
	--ödev -- yılı çekecek fonk bulmak için google kullan--
	...
	SELECT ROUND(AVG(Milliseconds)) as ortalama_süre
	FROM tracks;
